...
export type TRPCProvider<TRouter extends AnyRouter> = (
  props: TRPCProviderProps<TRouter> & {
...
export function createHooksInternal<TRouter extends AnyRouter>(config: CreateTRPCSolidOptions<TRouter>) {
  ...
  const Context = (config?.context ?? TRPCContext) as Context<TRPCContextState<TRouter>>;
  // const SolidQueryContext = config?.solidQueryContext as Context<QueryClient | undefined>;
  ...
  const TRPCProvider: TRPCProvider<TRouter> = props => {
    ...
    return (
        ...
        value={{
          ...
          fetchQuery: (pathAndInput, opts) => {
            return queryClient.fetchQuery({
              ...
              queryFn: () => (props.client as any).query(...getClientArgs(pathAndInput, opts)),
              ...opts,
            });
          },
          fetchInfiniteQuery: (pathAndInput, opts) => {
            return queryClient.fetchInfiniteQuery({
              ...
              queryFn: ({ pageParam }) => {
                ...
                return (props.client as any).query(...getClientArgs([path, actualInput], opts));
              },
              ...opts,
            });
          },
      ...
      >
        <QueryClientProvider client={queryClient} {...((props.queryClientOpts ?? {}) as any)}>
          {props.children}
        </QueryClientProvider>
      </Context.Provider>
    );
  ...
  >(
    ...
    console.log('invalidate', pathAndInput, filters, options);
    return config.queryClient.invalidateQueries({
      queryKey: getArrayQueryKey(pathAndInput),
      ...filters,
      ...options,
    });
  ...
  >(
    ...
    return config.queryClient.prefetchQuery({
      queryKey: getArrayQueryKey(pathAndInput),

...
import { insertImplicitAND, type ProcessedTokenResult, toFlattened } from '../evaluator.ts';
import { parseSearch, Token, type TokenResult } from '../parser.ts';
...
function assertTokens(tokens: TokenResult<Token>[] | null): asserts tokens is TokenResult<Token>[] {
  if (tokens === null) {
  ...
  describe('injects implicit AND', () => {
    describe('boolean operators', () => {
      ...
      it('multiple implicit AND', () => {
        const tokens = toFlattened(parseSearch('is:unresolved duration:>1h duration:<1m')!);
        const withImplicitAND = insertImplicitAND(tokens);
        expect(tokensToString(withImplicitAND)).toBe('is:unresolved AND duration:>1h AND duration:<1m');
      });
      ...
      it('multiple explicit OR', () => {
        const tokens = toFlattened(parseSearch('is:unresolved OR duration:>1h OR duration:<1h')!);
        const withImplicitAND = insertImplicitAND(tokens);
        expect(tokensToString(withImplicitAND)).toBe('is:unresolved OR duration:>1h OR duration:<1h');
      });
    ...
    describe('complex expressions', () => {
      it('handles complex expressions', () => {
        const tokens = toFlattened(parseSearch('is:unresolved AND ( duration:>1h OR duration:<1h )')!);
        expect(tokensToString(tokens)).toBe('is:unresolved AND ( duration:>1h OR duration:<1h )');
      });

      it('handles complex expressions with implicit AND', () => {
        const tokens = toFlattened(parseSearch('is:unresolved ( duration:>1h OR ( duration:<1h duration:1m ) )')!);
        const withImplicitAND = insertImplicitAND(tokens);

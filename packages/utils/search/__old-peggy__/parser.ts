 ...
 * https://github.com/getsentry/sentry/blob/4bffd8a9615296c134447e322769be8b068137c0/static/app/components/searchSyntax/parser.tsx
 */
...
import { getKeyName } from './utils.ts';
...
export class TokenConverter {
   ...
   */
  keyValidation = {
    isNumeric: (key: string) => this.config.numericKeys.has(key),
    ...
    isDuration: (key: string) => this.config.durationKeys.has(key),
  ...
  tokenKeyExplicitTag = (prefix: string, key: ReturnType<TokenConverter['tokenKeySimple']>) => ({
    ...this.defaultTokenFields,
  ...
  tokenValueRelativeDate = (value: string, sign: '-' | '+', unit: 'w' | 'd' | 'h' | 'm') => ({
    ...this.defaultTokenFields,
  ...
  tokenValueDuration = (value: string, unit: 'ms' | 's' | 'min' | 'm' | 'hr' | 'h' | 'day' | 'd' | 'wk' | 'w') => ({
    ...this.defaultTokenFields,
   ...
   */
  predicateTextOperator = (key: TextFilter['key']) => this.config.textOperatorKeys.has(getKeyName(key));
  ...
  checkFilterWarning = <T extends FilterType>(key: FilterMap[T]['key']) => {
    ...
    const keyName = getKeyName(key as TokenResult<Token.KEY_SIMPLE | Token.KEY_EXPLICIT_TAG>);
   ...
   * Checks a filter against some non-grammar validation rules
   */
  checkInvalidFilter = <T extends FilterType>(filter: T, key: FilterMap[T]['key'], value: FilterMap[T]['value']) => {
    // Text filter is the "fall through" filter that will match when other
    // filter predicates fail.
    if (this.config.validateKeys && this.config.supportedTags && !this.config.supportedTags[key.text]) {
      return {
        type: InvalidReason.INVALID_KEY,
        reason: `Invalid key. "${key.text}" is not a supported search key.`,
      };
    }

    if (filter === FilterType.TEXT) {
      return this.checkInvalidTextFilter(key as TextFilter['key'], value as TextFilter['value']);
    }
  ...
  checkInvalidTextFilter = (key: TextFilter['key'], value: TextFilter['value']) => {
    ...
    if (this.keyValidation.isDate(keyName)) {
      ...
      return {
        ...
        reason: `Invalid date format. Expected +/-duration (e.g. +1h) or ISO 8601-like (e.g. ${example.slice(0, 10)} or ${example})`,
        expectedType: [FilterType.DATE, FilterType.SPECIFIC_DATE, FilterType.RELATIVE_DATE],
      };
    }
  ...
  checkInvalidInFilter = ({ items }: InFilter['value']) => {
    ...
    if (hasEmptyValue) {
      return {
        ...
        reason: this.config.invalidMessages[InvalidReason.EMPTY_VALUE_IN_LIST_NOT_ALLOWED],
      };
    }

    if (this.config.disallowWildcard && items.some(item => item.value.value.includes('*'))) {
      return {
        type: InvalidReason.WILDCARD_NOT_ALLOWED,
        reason: this.config.invalidMessages[InvalidReason.WILDCARD_NOT_ALLOWED],
      };
...
function parseDate(input: string): { value: Date } {
  const date = dayjs.utc(input).toDate();
...
function parseRelativeDate(input: string, { sign, unit }: { sign: '-' | '+'; unit: string }): { value: Date } {
...
function parseNumber(input: string, unit: 'k' | 'm' | 'b' | 'K' | 'M' | 'B'): { value: number } {
  let number = numeric(input);
...
export const defaultConfig: SearchConfig = {
  ...
  invalidMessages: {
    [InvalidReason.FREE_TEXT_NOT_ALLOWED]: 'Free text is not supported in this search',
    [InvalidReason.WILDCARD_NOT_ALLOWED]: 'Wildcards not supported in search',
    [InvalidReason.LOGICAL_OR_NOT_ALLOWED]: 'The OR operator is not allowed in this search',
    [InvalidReason.LOGICAL_AND_NOT_ALLOWED]: 'The AND operator is not allowed in this search',
    [InvalidReason.MUST_BE_QUOTED]: 'Quotes must enclose text or be escaped',
    [InvalidReason.FILTER_MUST_HAVE_VALUE]: 'Filter must have a value',
    [InvalidReason.INVALID_BOOLEAN]: 'Invalid boolean. Expected true, 1, false, or 0.',
    [InvalidReason.INVALID_FILE_SIZE]: 'Invalid file size. Expected number followed by file size unit suffix',
    [InvalidReason.INVALID_NUMBER]: 'Invalid number. Expected number then optional k, m, or b suffix (e.g. 500k)',
    [InvalidReason.EMPTY_VALUE_IN_LIST_NOT_ALLOWED]: 'Lists should not have empty values',
  },
} as const;

function tryParseSearch<T extends { config: SearchConfig }>(query: string, config: T): ParseResult | null {
  try {
 ...
 * will result in null.
 */
export function parseSearch(query: string, additionalConfig?: Partial<SearchConfig>): ParseResult | null {
  const config = (
    additionalConfig ? R.mergeDeep({ ...defaultConfig }, additionalConfig) : defaultConfig
  ) as SearchConfig;

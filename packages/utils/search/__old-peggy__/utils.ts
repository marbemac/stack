 ...
 * https://github.com/getsentry/sentry/blob/4bffd8a9615296c134447e322769be8b068137c0/static/app/components/searchSyntax/utils.tsx
 */

import type { LocationRange } from 'peggy';

import type { TokenResult } from './parser.ts';
import { allOperators, Token } from './parser.ts';
 ...
 */
export function treeResultLocator<T>({ tree, visitorTest, noResultValue }: TreeResultLocatorOpts): T {
  ...
  const nodeVisitor = (token: TokenResult<Token> | null) => {
    ...
    switch (token.type) {
      ...
      case Token.LOGIC_GROUP:
        return transform({
          ...token,
          inner: treeTransformer({ tree: token.inner, transform }),
        });
      case Token.KEY_AGGREGATE_ARGS:
        return transform({
          ...token,
          args: token.args.map(v => ({
            ...v,
            value: Array.isArray(v.value)
              ? treeTransformer({ tree: v.value as unknown as TokenResult<Token>[], transform })
              : nodeVisitor(v.value),
          })),
        });
      case Token.VALUE_NUMBER_LIST:
...
export function isWithinToken(node: { location: LocationRange } | null | undefined, position: number) {
  if (!node) {
    return false;
  }
